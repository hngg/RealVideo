...
int vid_stream_test_file(char*filename) {
    FILE			*pFile;
	NALU_t 			*mNALU;
	...
	do {
        ...
        int type = (frameType==H264_FRAME)?(mNALU->buf[4] & 0x1F):GET_H265_NAL_TYPE(mNALU->buf[4]);
...
int main( int argc, char ** argv )
{
    ...
    char extenName[12] = {0};
    get_file_exten(argv[1], extenName);
	...
	stream_send_test("127.0.0.1", 36123, "127.0.0.1", 38123, argv[1], frameType);
	//stream_send_rtcp_test("127.0.0.1", 36123, "127.0.0.1", 38123, frameType);
